...
class BicepParser(BaseIaCParser):
    ...
    def __init__(self):
        ...
        self.resource_patterns = [
            # Standard resource: resource <name> '<type>@<version>' = { ... }
            r'resource\s+(\w+)\s+[\'"]([^\'">]+?)@[\d\-\.]+[\'"]?\s*=\s*\{',
            # Resource without version: resource <name> '<type>' = { ... }
    ...
    def _extract_resources(self, content: str, file_path: Path) -> None:
        ...
        for pattern in self.resource_patterns:
            matches = re.finditer(pattern, content_no_comments, re.IGNORECASE | re.MULTILINE)
            
            for match in matches:
                symbolic_name = match.group(1).strip()
                resource_type = match.group(2).strip()
                # Normalize: strip API version suffix if present
                if '@' in resource_type:
                    resource_type = resource_type.split('@', 1)[0].strip()
                # Check if it's a Microsoft (Azure) resource
                if resource_type.startswith('Microsoft.'):
                    full_resource_id = f"{resource_type}#{symbolic_name}"
                    if full_resource_id in seen:
                        continue
                    seen.add(full_resource_id)
                    ...
                    resource_section = self._extract_resource_section(
                        content_no_comments, match.start()
                    )
